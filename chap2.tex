...
Xeon 2.27GHz processors. Each machine had 333GB of disk space available for the
calculations. To reduce variability between experiments, each experiment was
run a total of 3 times, once on each machine. Between experiment runs, we clear
the file system cache to reduce variations from caching. \\
...
\c{StorageBackend} format. In the MGH corpus the largest EDF file is 150GB
and files commonly range from 5GB-20GB. Thus, to understand how the different
...
varied chunk sizes in powers of 2 from 64MB to 128MB.
...
\c{StorageBackend}. Chunk sizes range in powers of 2 from 64MB to 128MB.
