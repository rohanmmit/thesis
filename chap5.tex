...
Python and then to C++ (see \ref{ch5:abandon}) and implementing a web based
...
discussed in \ref{ch5:abandon}, the performance of this layer can greatly
...
\section{Abandoned Designs}\label{ch5:abandon}
...
\subsection{Spectrogram Annotations}\label{ch5:annotations}
...
\subsection{Change Point Detection and Clustering}\label{ch5:cpd}

As the corpus of EEG data grows, marking annotations by hand as described in
\ref{ch5:annotations} is rather tedious.  A more automated way would be to scan
...
\subsection{Distributed Architecture}\label{ch5:dist-arch}
...
Given the changes proposed in \ref{ch5:annotations} and \ref{ch5:cpd} a
